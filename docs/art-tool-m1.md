...
CLIはモデルを作業ディレクトリ基準で探し、見つからなければ実行ファイルの場所へ切り替える。
この切替を防ぐため、推論の直前に作業ディレクトリ側のモデルの存在とハッシュを照合し、`model_files`へ記録する。
1回目は生成したモデル、2回目の再入力は配布モデルと一致しなければ開始しない。
...
| `time_mode` | `fixed`は固定入力ノイズ場。`smooth`は二つの固定入力ノイズ場を補間。`smooth-constant`は補間中も分散を保つ |
...
`smooth`は補間中央で分散が端点の半分になり、周期の半分ごとに強度が弱まる。
記録済みの`input-smooth`の意味を保つため、分散を一定にする方式は`smooth-constant`として追加した。
`smooth-constant`は二つの独立な場の混合を`√((1−a)²+a²)`で割り、端点では`smooth`と一致する。
...
仕上げの開始時に親の記録で元データ保全を確認し、未補正推論のファイルを復号して記録済みの画素と照合する。
...
色の仕上げはCPU処理で、標準449.6秒、強め516.0秒を要した（同時実行を含む参考値）。
標準`melt-16`と強め`melt-24`を採用し、A01・A02の受入とR10・M1を完了した。

## 承認後の精査と追加検証

2026-09-13の実装精査を受けて、次の検証と修正を追加した。

### 承認記録

制作者の判断は、Gitで管理する[判断記録](art-tool-m1-acceptance.json)へ移した。
採用候補、見送り候補と理由、理由の出典（制作者の発言か記録か）を保存する。
`build/`以下の承認記録と比較一覧は、この判断記録と実行記録から生成し直す。

```powershell
python -B scripts/art_acceptance.py
```

生成時に、採用2候補の最終ファイルを復号し、記録済みの画素、時刻、寸法、300フレームと照合する。
あわせて、親記録の不変、3フレーム試作との先頭一致、`source_color=1`、配布モデルのハッシュも確認する。
見送り40件は、初回27候補と未補正の5秒版3候補が制作者の発言、試作途中の10候補が記録に基づく。
`melt-8`の選外理由は記録に残っていないため、その旨を明記した。

### 重み変異の寄与

重み変異だけを0にし、入力ぼかしと色の仕上げを同じにした対照を実行した。

```powershell
python -B scripts/art_destruction_probe.py --baseline-dir build/art/m0-test --output-dir build/art/m1-ablation --select melt-16-weights-0 melt-24-weights-0
```

先頭、中間、末尾のフレームで、採用候補と対照を比べた。

| 候補 | 平均絶対差（8ビット） | 差が16以上のチャネルを含む画素 |
|---|---|---|
| `melt-16` | 17.1 | 43.1% |
| `melt-24` | 41.7 | 67.2% |

採用した見た目の変化は、入力ぼかしと色の仕上げだけでは再現できず、重み変異が寄与している。
対照の比較動画は[承認一覧](../build/art/m1-source-color/index.html)で採用候補と並べた。

### 色の仕上げの高速化

旧実装と同じ演算順を保ったまま、行単位の分割と輝度の再利用で高速化した。
`retain=0`では、使わない元フレームの拡大を省いた。
旧実装の式と一致することを回帰テストで確認し、採用2候補を再仕上げして全300フレームのハッシュが承認済みの出力と一致した。
色の仕上げ処理は標準437.5秒から183.3秒、強め497.9秒から185.5秒になった（GPU処理との同時実行を含む参考値）。

### 残る論点

R08で時間変化の対象を入力ノイズに決めたが、採用した`melt-16`と`melt-24`は入力ノイズと時間変化を使っていない。
採用レシピに時間変化を加えるかどうかと、その方式は、M2以降で制作者が判断する。
