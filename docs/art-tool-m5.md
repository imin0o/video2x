...
2026-09-14にR22とR23を完了し、R21は制作者の最終確認待ちとした。
...
M1で採用済みの`melt-16`と`melt-24`は、最終構成で再生成した冒頭区間との画素一致を確認する。
この一致は長い映像全体の再評価を代替しない。
時間変化を加えた5秒本番映像のちらつきと、実測待ち時間の許容判定を制作者の確認欄に残す。
自動検証に成功しても、未回答の確認項目を承認済みにはしない。
