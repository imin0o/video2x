...
2026-09-15の「R21 完了させて」という指示を、残っていたちらつきと待ち時間を含む現行構成の受入として[記録した](art-tool-m5-acceptance.json)。
これは前回の確認待ち事項を提示した後の完了指示に基づき、新たに視聴したとの申告を記録するものではない。
対象は記録済みの個人制作環境で、単フレーム約4.3〜4.6秒、5秒動画約200〜220秒の測定結果とする。

受入記録は検証環境、本番映像、レシピ、各検証結果のSHA256を保持する。
次のコマンドで一致を確認して、受入済みの確認ページを再生成できる。

```powershell
python -B scripts/art_release_report.py --output-dir build/art/m5-verified-final --decision docs/art-tool-m5-acceptance.json
```
