...
def prepare(spec, directory, baseline):
    """Decode from the start so GOP seek/decoder history cannot change overlapping frames."""
    clip = directory / 'input.mkv'
    pending = directory / 'input.partial.mkv'
    origin, start = Fraction(spec['info']['origin']), Fraction(str(spec['start']))
    end = Fraction(str(spec['end'])) if spec['end'] is not None else None
    selected, evidence, previous, next_time = [], [], None, None
    with av.open(spec['source']) as reader, av.open(str(pending), 'w') as writer:
        ...
        stream.width, stream.height = spec['info']['width'], spec['info']['height']
        ...
        for frame in reader.decode(source):
            ...
            duration = frame.duration * frame.time_base if frame.duration else 1 / (source.average_rate or 30)
            selected.append(dict(time=str(stamp), duration=str(duration), source_pts=frame.pts,
                                 source_time_base=str(frame.time_base)))
            evidence.append(signature(out))
            for packet in stream.encode(out):
                writer.mux(packet)
            progress('decode-source', len(selected))
        for packet in stream.encode():
            writer.mux(packet)
    if not selected:
        raise ValueError('The selected interval contains no video frame')
    tail = Fraction(selected[-1]['time']) + Fraction(selected[-1]['duration'])
    tail = next_time if next_time is not None else tail
    tail = min(tail, end) if end is not None else tail
    for index, frame in enumerate(selected):
        stop = Fraction(selected[index + 1]['time']) if index + 1 < len(selected) else tail
        frame['duration'] = str(stop - Fraction(frame['time']))
        if Fraction(frame['duration']) <= 0:
    ...
    context['source'] = dict(path=spec['source'], sha256=sha256(Path(spec['source'])),
                             start_seconds=spec['start'], duration_seconds=float(tail - start))
    context['prepared_input'] = dict(path=str(clip), sha256=sha256(clip),
                                     probe=dict(streams=[spec['info']]),
                                     source_times=[f['time'] for f in selected])
